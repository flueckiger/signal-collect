...
class VertexStorageSpecEx extends SpecificationWithJUnit with Mockito {

  sequential

  "Memory Efficient Vertex Storage" should {

    val storageFactory = new MemoryEfficientStorage[Int, Double]

    "hold all vertices inserted" in {
      val defaultMessageBus = mock[DefaultMessageBus[Any, Any]]
      val vertexList = List(new PageRankVertexEx(0, 1), new PageRankVertexEx(1, 1), new PageRankVertexEx(2, 1))
  ...
  "Java Map Vertex Storage" should {
    ...
    val storageFactory = new JavaMapStorage[Int, Double]
