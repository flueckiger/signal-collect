...
class DistributedSimulationSpecEx extends FlatSpec with ShouldMatchers with TestAnnouncements {
  ...
  "Signal/Collect" should "terminate with a low latency when run in a simulated distributed synchronous mode" in {
    ...
    val nodeActors = DistributedSimulatorEx.getNodeActors(numberOfSimulatedNodes, workersPerSimulatedNode, idleDetectionPropagationDelay)
    ...
    try {
      (1 to 10).foreach { i =>
        ...
        for (i <- 0 until circleLength) {
          g.addVertex(new PageRankVertexEx(i))
          g.addEdge(i, new PageRankEdgeEx((i + 1) % circleLength))
        }
        g.awaitIdle
        g.execute(new ExecutionConfiguration[Int, Double].withExecutionMode(ExecutionMode.Synchronous).withSignalThreshold(0.001))
