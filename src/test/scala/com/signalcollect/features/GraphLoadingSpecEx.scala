...
class GraphLoadingSpecEx extends SpecificationWithJUnit {
  ...
  "Graph" should {
    ...
    "support the loadGraph command" in {
      ...
      try {
        val graphLoaders = (1 to 100).map(x => (10 * x until ((10 * x) + 10)).toIterator.map(y => new PageRankVertexEx(y)).map(z => {
          ge: GraphEditor[Int, Double] =>
            ge.addVertex(z)
        }))
        ...
        graph.awaitIdle
        val stats = graph.execute(new ExecutionConfiguration[Int, Double].withSignalThreshold(0.01))
