...
import com.signalcollect.DataGraphVertexEx
import com.signalcollect.StateForwarderEdge
import com.signalcollect.DataFlowVertexEx
import com.signalcollect.GraphEditor
import akka.event.Logging

class CountingVertexEx(id: Int) extends DataGraphVertexEx[Int, (Int, Int), Int, (Int, Int)](id, (0, 0)) {
  type Signal = (Int, Int)
  def collect = (state._1, state._2 + 1)

  override def deliverSignalWithSourceId(signal: Signal, sourceId: Int, graphEditor: GraphEditor[Int, (Int, Int)]): Boolean = {
...
class ComputationTerminationSpecEx extends SpecificationWithJUnit with Mockito {

  def createPageRankCircleGraph(vertices: Int): Graph[Int, Double] = {
    ...
    for (id <- idSet) {
      graph.addEdge(id, new PageRankEdgeEx((id % vertices) + 1))
    }
    graph
  }

  def createCountingCircleGraph(vertices: Int): Graph[Int, (Int, Int)] = {
    val graph = new GraphBuilder[Int, (Int, Int)].build //.withLoggingLevel(Logging.DebugLevel)
    val idSet = (1 to vertices).toSet
    for (id <- idSet) {
      graph.addVertex(new CountingVertexEx(id))
    }
    for (id <- idSet) {
  ...
  "Eager convergence detection" should {
    "not suffer from spurious idle detections" in {
      ...
      try {
        val steps = 2000
        g.execute(new ExecutionConfiguration[Int, (Int, Int)].withExecutionMode(ExecutionMode.Synchronous).withStepsLimit(steps))
  ...
  "Steps limit" should {
    ...
    "work for synchronous computations" in {
      ...
      for (i <- 1 to 10) {
        ...
        try {
          val execConfig = new ExecutionConfiguration[Int, Double]
            .withSignalThreshold(0)
            .withStepsLimit(1)
            .withExecutionMode(ExecutionMode.Synchronous)
          val info = graph.execute(execConfig)
          val state = graph.forVertexWithId(1, (v: PageRankVertexEx[Any]) => v.state)
  ...
  "Global convergence" should {
    ...
    "work for synchronous computations" in {
      ...
      try {
        case object GlobalTermination extends GlobalTerminationDetection[Int, Double] {
          override val aggregationInterval: Long = 1
          def shouldTerminate(g: Graph[Int, Double]) = {
