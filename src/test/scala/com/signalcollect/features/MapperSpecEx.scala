...
class Worker0MapperEx[Id] extends VertexToWorkerMapper[Id] {
  def getWorkerIdForVertexId(vertexId: Id): Int = 0
  def getWorkerIdForVertexIdHash(vertexIdHash: Int): Int = 0
}

class Worker0MapperFactoryEx[Id] extends MapperFactory[Id] {
  def createInstance(numberOfNodes: Int, workersPerNode: Int) = new Worker0MapperEx
}
...
class MapperSpecEx extends SpecificationWithJUnit with Serializable {
  ...
  "Custom mapper" should {
    "correctly support PageRank computation" in {
      ...
      val graph = new GraphBuilder[Int, Double].withMapperFactory(new Worker0MapperFactoryEx[Int]).build
      try {
        for (i <- 0 until 5) {
