...
class BulkSignalingSpecEx extends SpecificationWithJUnit with Serializable {
  ...
  "Bulk signaling" should {
    "deliver correct results on a 5-cycle graph" in {
      ...
      val graph = new GraphBuilder[Int, Double].withMessageBusFactory(new BulkAkkaMessageBusFactory(1000, false)).build
      try {
        for (i <- 0 until 5) {
    ...
    "handle a bulk size of 1 correctly" in {
      ...
      val graph = new GraphBuilder[Int, Double].withMessageBusFactory(new BulkAkkaMessageBusFactory(1, true)).build
      try {
        for (i <- 0 until 5) {
          val v = new PageRankVertexEx(i)
          graph.addVertex(v)
        ...
        graph.execute(new ExecutionConfiguration[Int, Double].withExecutionMode(ExecutionMode.PureAsynchronous).withCollectThreshold(0).withSignalThreshold(0.00001))
        var allcorrect = graph.aggregate(new ModularAggregationOperation[Boolean] {
          val neutralElement = true
          def aggregate(a: Boolean, b: Boolean): Boolean = a && b
          def extract(v: Vertex[_, _, _, _]): Boolean = pageRankFiveCycleVerifier(v)
