...
class GraphModificationSpecEx extends SpecificationWithJUnit {
  ...
  "GraphEditor" should {
    "support modification functions" in {
      ...
      try {
        graph.modifyGraph({ _.addVertex(new GraphModificationVertexEx(0, 1)) }, Some(0))
        graph.modifyGraph({ _.addVertex(new GraphModificationVertexEx(1, 1)) }, Some(1))
        graph.modifyGraph({ _.addVertex(new GraphModificationVertexEx(2, 1)) }, Some(2))
        graph.modifyGraph({ _.addVertex(new GraphModificationVertexEx(3, 1)) }, Some(3))
        ...
        graph.aggregate(new CountVertices[GraphModificationVertexEx]) === 4
        ...
        graph.aggregate(new CountVertices[GraphModificationVertexEx]) === 2
    ...
    "keep accurate statistics when using individual vertex removals" in {
      val graph = GraphBuilder.build
      try {
        graph.addVertex(new GraphModificationVertexEx(0, 1))
        graph.addVertex(new GraphModificationVertexEx(1, 1))
        graph.addVertex(new GraphModificationVertexEx(2, 1))
        graph.addVertex(new GraphModificationVertexEx(3, 1))
...
class GraphModificationVertexEx(id: Int, state: Int) extends DataGraphVertexEx[Int, Int, Any, Any](id, state) {
  def collect = 1
}