 ...
 */
class SegregationAgentEx(id: Any, initialState: Int, equalityThreshold: Float) extends DataGraphVertexEx[Any, Int, Any, Any](id, initialState) {
  type Signal = Int
...
object SchellingSegregationEx extends App {
  ...
  for (column <- 0 to columns; row <- 0 to rows) {
    graph.addVertex(new SegregationAgentEx((column, row), (math.random * 2.0).floor.toInt, 0.45f))
  }
