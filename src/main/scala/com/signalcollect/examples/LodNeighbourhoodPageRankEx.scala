...
class LodNeighbourhoodPageRankEx(id: String, crawlDepth: Int = 3) extends PageRankVertexEx[String](id) {

  override def afterInitialization(ge: GraphEditor[String, Double]) {
    println("Added " + id)
    if (crawlDepth > 0) {
      try {
        ...
        for (dbpediaUrl <- urls) {
          ge.addVertex(new LodNeighbourhoodPageRankEx(dbpediaUrl, crawlDepth - 1))
...
object LodNeighbourhoodEx extends App {
  val graph = new GraphBuilder[String, Double].build
  graph.addVertex(
    new LodNeighbourhoodPageRankEx("http://dbpedia.org/resource/Tiger"))
