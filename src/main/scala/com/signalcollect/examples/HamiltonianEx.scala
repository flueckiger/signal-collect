 ...
 */
object HamiltonianEx extends App {
  val graph = new GraphBuilder[String, Map[List[String], Int]].build

  /**
   * Still need to test performance on complete and larger graphs
   */

  graph.addVertex(new HamiltonianVertexEx("a", Map(List("a") -> 0)))
  graph.addVertex(new HamiltonianVertexEx("b", Map(List("b") -> 0)))
  graph.addVertex(new HamiltonianVertexEx("c", Map(List("c") -> 0)))
  graph.addVertex(new HamiltonianVertexEx("d", Map(List("d") -> 0)))
  graph.addVertex(new HamiltonianVertexEx("e", Map(List("e") -> 0)))

  graph.addEdge("a", new HamiltonianEdgeEx("d", 3))
  graph.addEdge("d", new HamiltonianEdgeEx("a", 3))
  graph.addEdge("a", new HamiltonianEdgeEx("b", 1))
  graph.addEdge("b", new HamiltonianEdgeEx("a", 1))
  graph.addEdge("d", new HamiltonianEdgeEx("b", 2))
  graph.addEdge("b", new HamiltonianEdgeEx("d", 2))
  graph.addEdge("d", new HamiltonianEdgeEx("c", 1))
  graph.addEdge("c", new HamiltonianEdgeEx("d", 1))
  graph.addEdge("b", new HamiltonianEdgeEx("c", 1))
  graph.addEdge("c", new HamiltonianEdgeEx("b", 1))

  // a problem with isolated vertices is that it is not able to find hamiltonian paths depending on the starting vertex
  graph.addEdge("e", new HamiltonianEdgeEx("a", 1))
  graph.addEdge("a", new HamiltonianEdgeEx("e", 1))
 ...
 *
 */
class HamiltonianVertexEx(vertexId: String, initialState: Map[List[String], Int]) extends DataGraphVertexEx[String, Map[List[String], Int], String, Map[List[String], Int]](vertexId, initialState) {
 ...
 */
class HamiltonianEdgeEx(t: String, w: Int) extends OnlySignalOnChangeEdge(t) {

  override def weight: Double = w

  type Source = HamiltonianVertexEx
