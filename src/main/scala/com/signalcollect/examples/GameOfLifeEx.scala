...
class GameOfLifeCellEx(id: Any, initialState: Int)
    extends DataGraphVertexEx[Any, Int, Any, Int](id, initialState) {
  type Signal = Int
  def collect = {
 ...
 */
object GameOfLifeEx extends App {
  val graph = new GraphBuilder[Any, Int].withConsole(true, 8090)
  ...
  for (column <- 0 to columns; row <- 0 to rows) {
    graph.addVertex(new GameOfLifeCellEx((column, row), (math.random * 2.0).floor.toInt))
  }
  ...
  val execConfig = new ExecutionConfiguration[Any, Int].withExecutionMode(ExecutionMode.Interactive)
