 ...
 *
 */
class SudokuAssociationEx(t: Int) extends OptionalSignalEdge(t) {
 ...
 *
 */
class SudokuCellEx(id: Int, initialState: Option[Int] = None) extends DataGraphVertexEx[Int, Option[Int], Int, Double](id, initialState) {

  type Signal = Int

  var possibleValues = SudokuHelperEx.legalNumbers
...
object SudokuEx extends App {
  ...
  SudokuHelperEx.printSudoku(seed)
  ...
  graph.shutdown
  SudokuHelperEx.printSudoku(result)
   ...
   */
  def tryPossibilities(graph: Graph[Int, Double]): Graph[Int, Double] = {

    val possibleValues = new ListMap[Int, Set[Int]]()
    graph.foreachVertex(v => possibleValues.put(v.id.asInstanceOf[Int], v.asInstanceOf[SudokuCellEx].possibleValues))
    graph.shutdown
  ...
  def computeGraphFactory(seed: Map[Int, Int]): Graph[Int, Double] = {
    val graph = new GraphBuilder[Int, Double].build

    //Add all Cells for Sudoku
    for (index <- 0 to 80) {
      val seedValue = seed.get(index)
      graph.addVertex(new SudokuCellEx(index, seedValue))
    }

    //Determine neighboring cells for each cell and draw the edges between them
    for (index <- 0 to 80) {
      SudokuHelperEx.cellsToConsider(index).foreach({ i =>
        graph.addEdge(i, new SudokuAssociationEx(index))
      })
    }
    graph
  }
