 ...
 */
class CWVertexEx(id: Any, selfPreference: Double = 1.0) extends DataGraphVertexEx[Any, Any, Any, (Any, Double)](id, id) {
 ...
 */
class CWEdgeEx(t: Any, weight: Double = 1.0) extends DefaultEdge(t) {
  type Source = CWVertexEx
 ...
 */
object ChineseWhispersClusteringEx extends App {

  val graph = new GraphBuilder[Any, (Any, Double)].build

  for (i <- 0 until 11) {
    graph.addVertex(new CWVertexEx(i))
  }

  graph.addVertex(new CWVertexEx(1))
  graph.addVertex(new CWVertexEx(2))
  graph.addVertex(new CWVertexEx(3))
  graph.addVertex(new CWVertexEx(4))
  graph.addVertex(new CWVertexEx(5))

  for (i <- 0 until 2) {
    graph.addEdge(i, new CWEdgeEx(i + 1))
    graph.addEdge(i + 1, new CWEdgeEx(i))
  }

  graph.addEdge(0, new CWEdgeEx(2))
  graph.addEdge(2, new CWEdgeEx(0))
  ...
  graph.addEdge(10, new CWEdgeEx(8))
  graph.addEdge(8, new CWEdgeEx(10))
